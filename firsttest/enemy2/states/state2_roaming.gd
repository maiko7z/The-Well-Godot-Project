...
@export var _roaming_speed : int = 8.0
...
func _ready() -> void:
	...
	_nav_map = _enemy2.get_world_3d().get_navigation_map()
...
func enter(previous_state_name: String, data := {}) -> void:
	...
	if data.has("do_not_reset_path") and data["do_not_reset_path"]:
		_enemy2.travel_to_position(_enemy2.navigation_agent.target_position, 8.0)
		return
...
func _travel_to_random_position() -> void:
	...
	_enemy2.travel_to_position(rand_pos, 8.0)
