...
@export var search_time : int = 5.0
@export var _searching_speed : int = 1.0
...
func physics_update(_delta: float) -> void:
	...
	if _enemy2.navigation_agent.is_navigation_finished():
		_go_to_position_around_player_last_seen_position()
		
	
	if not _enemy2.is_line_of_sight_broken():
		if vec_to_player.length() < 15.0:
			if _enemy2.player.get_node("Head/SpotLight3D").visible:
				#print("I see your light!")
				requested_transition_to_other_state.emit("Chasing")
			if vec_to_player.length() < 5.0 and not _enemy2.player.get_node("Head/SpotLight3D").visible and _enemy2.player.get_node("Head/Camera3D").v_offset == 0:
				...
				requested_transition_to_other_state.emit("Chasing")
			if _enemy2.is_player_conspicuous():
				#print("I hear you!")
				requested_transition_to_other_state.emit("Chasing")
...
func _go_to_position_around_player_last_seen_position() -> void:
	...
	_enemy2.travel_to_position(random_position, _searching_speed, true)
