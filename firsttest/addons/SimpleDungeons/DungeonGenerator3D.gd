...
func _ready():
	...
	$"..".connect("map_ready_to_generate", generate)
...
func get_room_at_pos(grid_pos : Vector3i) -> DungeonRoom3D:
		...
		## Nate note: This shit is BUGGED AND WILL CAUSE AN ERROR IF WORLD.TSCN IS NOT THE MAIN SCENE IDK GOD HELP US
		#var quick_check = _quick_room_check_dict.get(grid_pos)
		#if quick_check == null:
			#print("check was null!")
			##return quick_check
